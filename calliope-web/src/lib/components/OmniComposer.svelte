<script lang="ts">
	...
	interface Props {
		...
		allowUpload?: boolean;
		targetCharacters?: Array<{ name: string; role?: string | null }>;
	...
	function getTargetName(input: ComfyDynamicInput, index: number): string | null {
		const role = normalizeInputRole(input.role);
		if (role === 'character') {
			return targetCharacters[index]?.name ?? targetCharacters[0]?.name ?? null;
		}
		if (role === 'location') {
		...
		if (role === 'image') {
			if (index < targetCharacters.length) {
				return targetCharacters[index].name;
			}
			if (index === targetCharacters.length && targetLocation) {
